...
describe('文件管理操作', () => {
  ...
  describe('文件列表操作', () => {
    ...
    it('应获取文件信息', async () => {
      const file = await fileService.getFileInfo('/workspace/我的第一张卡片.card');

      expect(file).toBeDefined();
      expect(file?.name).toBe('我的第一张卡片.card');
  ...
  describe('文件删除', () => {
    it('应删除文件', async () => {
      // 先创建文件
      await fileService.createCard({
        name: '待删除卡片',
        parentPath: '/workspace',
      });

      const eventHandler = vi.fn();
      eventEmitter.on('file:deleted', eventHandler);

      const result = await fileService.deleteFile('/workspace/待删除卡片.card');
  ...
  describe('文件重命名', () => {
    it('应重命名文件', async () => {
      const eventHandler = vi.fn();
      eventEmitter.on('file:renamed', eventHandler);

      const result = await fileService.renameFile(
        '/workspace/我的第一张卡片.card',
        '重命名的卡片'
      );

      expect(result.success).toBe(true);
      expect(result.file?.name).toBe('重命名的卡片.card');
      expect(eventHandler).toHaveBeenCalled();
    });
    ...
    it('应保留原有扩展名', async () => {
      ...
      expect(result.file?.name).toBe('新名称.box');
    });
  ...
  describe('文件复制', () => {
    it('应复制文件', async () => {
      const eventHandler = vi.fn();
      eventEmitter.on('file:copied', eventHandler);
  ...
  describe('文件移动', () => {
    it('应移动文件', async () => {
      ...
      const eventHandler = vi.fn();
      eventEmitter.on('file:moved', eventHandler);

      const result = await fileService.moveFile(
        '/workspace/待移动卡片.card',
        '/workspace/学习笔记'
      );
  ...
  describe('文件夹展开/折叠', () => {
    ...
    it('应设置文件夹展开状态', async () => {
      ...
      const folder = await fileService.getFileInfo('/workspace/项目文档');
  ...
  describe('剪贴板操作', () => {
    ...
    it('应粘贴文件', async () => {
      fileService.copyToClipboard(['/workspace/我的第一张卡片.card']);

      const results = await fileService.paste('/workspace/学习笔记');
  ...
  describe('文件搜索', () => {
    it('应搜索文件', async () => {
      const results = await fileService.searchFiles('卡片');
    ...
    it('应按类型过滤搜索结果', async () => {
      const results = await fileService.searchFiles('card', { type: 'card' });
