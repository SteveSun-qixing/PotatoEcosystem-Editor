...
│  │                                                             │ │
│  │   ┌────────────┐ ┌────────────┐ ┌──────────┐ ┌──────────────┐│ │
│  │   │EditorStore │ │ CardStore  │ │ UIStore  │ │SettingsStore ││ │
│  │   │ 编辑器状态  │ │  卡片状态   │ │ UI 状态  │ │ 引擎设置状态  ││ │
│  │   └────────────┘ └────────────┘ └──────────┘ └──────────────┘│ │
│  │                                                             │ │
│  └────────────────────────────────────────────────────────────┘ │
│                              ↓                                   │
...
### SettingsStore 引擎设置状态

管理引擎级别的全局设置。采用「注册中心」架构，Store 本身不知道具体有哪些设置分类，所有分类信息通过 `SettingsPanelDefinition` 动态注册。

```typescript
const useSettingsStore = defineStore('settings', {
  state: () => ({
    // 面板定义注册表: categoryId -> SettingsPanelDefinition
    panelRegistry: {} as Record<string, SettingsPanelDefinition>,
    // 设置数据存储: categoryId -> 该分类的数据
  ...
  getters: {
    sortedCategories: /* 按 order 排序的分类列表 */,
    groupedCategories: /* 按 group 分组的分类列表 */,
    categoryCount: /* 已注册分类数量 */,
  },

  actions: {
    // 面板注册
    registerPanel(definition),
    registerPanels(definitions),
    unregisterPanel(categoryId),
    hasPanel(categoryId),
    getPanelComponent(categoryId),
    ...
    // 持久化
    exportAll(),    // 导出所有设置（用于保存）
    importAll(data), // 导入设置（用于恢复）
    markInitialized(),
  },
});
```

**核心设计**：`updateData` 内部自动调用对应面板定义的 `onChange` 处理器，实现副作用（如切换主题调用 SDK.ThemeManager）与数据变更的自动绑定。

**Dock 设置入口**：程序坞（Dock）组件末尾增加了固定的齿轮图标按钮，与工具窗口图标分隔开。点击后通过事件链 `Dock -> WindowLayer -> InfiniteCanvas -> App.vue` 打开全屏设置弹窗。
...
│                           ↓ 调用 actions                      │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐    │
│  │ Editor   │ │  Card    │ │   UI     │ │  Settings    │    │
│  │ Store    │ │  Store   │ │  Store   │ │  Store       │    │
│  │          │ │          │ │          │ │              │    │
│  │ state    │ │ state    │ │ state    │ │ panelRegistry│    │
│  │ getters  │ │ getters  │ │ getters  │ │ settingsData │    │
